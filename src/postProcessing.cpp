...
// visualize the ETF
void PP::ETF(Mat &flowfield, Mat &dis , int s,int noiseupsample) {
	//Mat noise = Mat::zeros(cv::Size(flowfield.cols / 2, flowfield.rows / 2), CV_32F);
	Mat noise = Mat::zeros(cv::Size(flowfield.cols/noiseupsample, flowfield.rows/noiseupsample), CV_32F);
	dis = Mat::zeros(flowfield.size(), CV_32F);
	randu(noise, 0, 1.0f);
	//resize(noise, noise, flowfield.size(), 0, 0, INTER_NEAREST);
    resize(noise, noise, flowfield.size(), 0, 0, INTER_CUBIC);
...
// visualize the ETF
void PP::ETF_image(Mat &input, Mat &flowfield, Mat &dis , int s) {
    input.convertTo(input, CV_32FC3, 1.0 / 255.0);
	dis = Mat::zeros(input.size(), CV_32FC3);
	...
	for (int i = 0; i < nRows; i++) {
		for (int j = 0; j < nCols; j++) {
			...
			for (int k = 0; k < s; k++) {
				...
				int xx = (int(x) + nRows) % nRows;
				int yy = (int(y) + nCols) % nCols;
				dis.at<Vec3f>(i, j) += w*input.at<Vec3f>(xx, yy);
				w_sum += w;
			}
			...
			for (int k = 0; k < s; k++) {
				...
				float r2 = k*k;
				float w = (1 / (M_PI*sigma))*exp(-(r2) / sigma);
				dis.at<Vec3f>(i, j) += w*input.at<Vec3f>(int(x + nRows) % nRows, int(y + nCols) % nCols);
				w_sum += w;
			}
			dis.at<Vec3f>(i, j) /= w_sum;
		}
	}
}
